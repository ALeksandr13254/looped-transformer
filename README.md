...
`model.n_last_tokens` - отвечает за то, сколько последних токенов, подаваемых на вход каждого слоя, будет использоваться. Если задать значение `0`, то модель не будет использовать токены со входа. Если задать положительное целочисленное число, то возьмет со входа указанное количвество токенов начиная с конца, замаскировав предыдущие токены. Если задать отрицательное целочисленное число (например `-1`), то все токены со входа будут использоваться.

`model.use_lstm_layer` - отвечает за то, будут ли использоваться слои LSTM (значение `True`) или decoder-only transformer блоки (значение `False`).
