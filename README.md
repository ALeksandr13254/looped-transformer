...
Для обучения была использована реализация из оригинальной статьи https://github.com/leiay/looped_transformer, но с уменьшением числа шагов обучения и размерности регрессии. Также в исходный код были добавлены параметры `model.n_last_tokens` и `model.use_lstm_layer`, которые можно вызывать из конфига модели для обучения.

- **model.n_last_tokens**: Параметр, определяющий количество последних токенов, которые будут подаваться на вход каждого слоя модели.
  - Если задать значение `0`, модель не будет использовать входные токены.
  - Если задать положительное целочисленное значение, то модель возьмет указанное количество последних токенов, игнорируя предыдущие.
  - Если задать отрицательное целочисленное значение (например, `-1`), модель будет использовать все входные токены без исключения.

- **model.use_lstm_layer**: Параметр, определяющий, какие слои будут использоваться в модели.
  - При значении `True`, будут использоваться слои LSTM.
  - При значении `False`, будут использоваться блоки decoder-only transformer.
...
**Процесс проведения:**
Для проверки этой гипотезы были проведены эксперименты по маскировке части выходного состояния looped transformer для каждой итерации, оставляя только n последних токенов. Для этого был доработан исходный код первоначального исследования, и в конфиг был добавлен параметр model.n_last_tokens, который отвечает за то, сколько последних токенов, подаваемых на вход каждого слоя, будет использоваться. Затем были обучены модели с параметром model.n_last_tokens равным: 20, 55 и 75.
...
**Процесс проведения:**
Для проверки данной гипотезы было проведено обучение моделей looped transformer с 1, 2 и 3 слоями, для этого в конфиге модели устанавливалось значение параметра model.n_layer равным: 1, 2 и 3.
