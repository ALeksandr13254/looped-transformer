...
- **Общий вывод**: Замена блоков transformer на LSTM слои приводит к значительному ухудшению качества модели. Это свидетельствует о том, что LSTM слои менее подходят для задач, требующих интеграции и обработки информации в рамках архитектуры looped transformer. decoder-only transformer блоки обеспечивают более высокую точность, что делает их предпочтительным выбором для таких задач.
